...
# 挑 k 個 index, 讓 nums1 對應的 k 個數相加，再乘min(nums2對應k個數) 希望最大
class Solution:
    def maxScore(self, nums1: List[int], nums2: List[int], k: int) -> int:
        ...
        heap = [a[i][1] for i in range(k)] # 找到最前面的 k 組數字，加入heap資料結構
        heapify(heap) # 前k項的nums1的這k個數，換加入新的n1,n2組
        total = sum(heap) 
        ans = total * a[k-1][0] # 前k項nums1及對應最小的nums2相乘
